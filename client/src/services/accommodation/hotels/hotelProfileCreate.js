// frontend/src/services/accommodation/hotels/hotelProfileCreate.js
...
const HotelProfileCreate = () => {
  ...
  useEffect(() => {
    // Fetch sub-service name based on subServiceId
    const fetchSubService = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/subservices/${subServiceId}`);
        setSubServiceName(response.data.name);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/profiles/hotels`, profileData, {
        headers: { 
  ...
  return (
    ...
    >
      <Container sx={{ flexGrow: 1, py: 4 }}>
        <Typography variant="h4" gutterBottom>{`Create Profile for ${subServiceName}`}</Typography>
        <Paper sx={{ p: 4, mt: 2 }}>
...
export default HotelProfileCreate;
